...
function displayName(firstName?: string, lastName?: string) {
  return [firstName, lastName].filter((value): value is string => Boolean(value?.trim())).join(" ").replace(/\s+/g, " ").trim() || "Foydalanuvchi";
}
...
export async function getEngagement(contentId: string, voterId?: string, sort: "latest" | "top" = "latest", offset = 0) {
  ...
  const [ratings, reactions, rows, totals] = await Promise.all([
    sql`SELECT COALESCE(AVG(score), 0)::float AS average, COUNT(*)::int AS count, COALESCE(MAX(score) FILTER (WHERE voter_id = ${voterId ?? ""}), 0)::int AS "myScore" FROM content_ratings WHERE content_id = ${contentId} AND voter_id LIKE 'user:%'` as unknown as Promise<{ average: number; count: number; myScore: number }[]>,
    sql`SELECT COUNT(*) FILTER (WHERE reaction = 'LIKE')::int AS likes, COUNT(*) FILTER (WHERE reaction = 'DISLIKE')::int AS dislikes, COALESCE(MAX(reaction) FILTER (WHERE voter_id = ${voterId ?? ""}), '') AS "myReaction" FROM content_reactions WHERE content_id = ${contentId} AND voter_id LIKE 'user:%'` as unknown as Promise<{ likes: number; dislikes: number; myReaction: "LIKE" | "DISLIKE" | "" }[]>,
    sort === "top"
      ? sql`SELECT c.id, c.body, c.parent_id AS "parentId", c.created_at AS "createdAt", COALESCE(u.id, CONCAT('guest:', c.id)) AS "authorId", u.first_name AS "firstName", u.last_name AS "lastName", c.guest_name AS "guestName", u.telegram_photo_url AS "telegramPhotoUrl", COALESCE(u.role, 'GUEST') AS role, COUNT(cl.comment_id)::int AS likes, BOOL_OR(cl.user_id = ${voterId ?? ""}) AS "likedByMe" FROM comments c LEFT JOIN users u ON u.id = c.user_id LEFT JOIN comment_likes cl ON cl.comment_id = c.id WHERE c.content_id = ${contentId} AND c.status = 'APPROVED' GROUP BY c.id, u.id, c.guest_name ORDER BY (c.parent_id IS NOT NULL), COUNT(cl.comment_id) DESC, c.created_at DESC LIMIT 10 OFFSET ${offset}` as unknown as Promise<EngagementCommentRow[]>
      : sql`SELECT c.id, c.body, c.parent_id AS "parentId", c.created_at AS "createdAt", COALESCE(u.id, CONCAT('guest:', c.id)) AS "authorId", u.first_name AS "firstName", u.last_name AS "lastName", c.guest_name AS "guestName", u.telegram_photo_url AS "telegramPhotoUrl", COALESCE(u.role, 'GUEST') AS role, COUNT(cl.comment_id)::int AS likes, BOOL_OR(cl.user_id = ${voterId ?? ""}) AS "likedByMe" FROM comments c LEFT JOIN users u ON u.id = c.user_id LEFT JOIN comment_likes cl ON cl.comment_id = c.id WHERE c.content_id = ${contentId} AND c.status = 'APPROVED' GROUP BY c.id, u.id, c.guest_name ORDER BY (c.parent_id IS NOT NULL), c.created_at DESC LIMIT 10 OFFSET ${offset}` as unknown as Promise<EngagementCommentRow[]>,
    sql`SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE parent_id IS NULL)::int AS "topLevel" FROM comments WHERE content_id = ${contentId} AND status = 'APPROVED'` as unknown as Promise<{ total: number; topLevel: number }[]>,
  ]);
...
export async function saveRating(contentId: string, voterId: string, score: number) {
  await ensureEngagementTables(); const sql = db();
  const inserted = await sql`INSERT INTO content_ratings (content_id, voter_id, score) VALUES (${contentId}, ${voterId}, ${score}) ON CONFLICT (content_id, voter_id) DO NOTHING RETURNING score` as unknown[];
  return inserted.length > 0;
}

export async function saveReaction(contentId: string, voterId: string, reaction: "LIKE" | "DISLIKE") {
  await ensureEngagementTables(); const sql = db();
  const inserted = await sql`INSERT INTO content_reactions (content_id, voter_id, reaction) VALUES (${contentId}, ${voterId}, ${reaction}) ON CONFLICT (content_id, voter_id) DO NOTHING RETURNING reaction` as unknown[];
  return inserted.length > 0;
}
