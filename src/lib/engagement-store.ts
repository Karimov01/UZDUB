...
export async function getEngagement(contentId: string, voterId?: string, sort: "latest" | "top" = "latest", offset = 0) {
  ...
  const [ratings, reactions, rows, totals] = await Promise.all([
    sql`SELECT COALESCE(AVG(score), 0)::float AS average, COUNT(*)::int AS count, COALESCE(MAX(score) FILTER (WHERE voter_id = ${voterId ?? ""}), 0)::int AS "myScore" FROM content_ratings WHERE content_id = ${contentId}` as unknown as Promise<{ average: number; count: number; myScore: number }[]>,
    sql`SELECT COUNT(*) FILTER (WHERE reaction = 'LIKE')::int AS likes, COUNT(*) FILTER (WHERE reaction = 'DISLIKE')::int AS dislikes, COALESCE(MAX(reaction) FILTER (WHERE voter_id = ${voterId ?? ""}), '') AS "myReaction" FROM content_reactions WHERE content_id = ${contentId}` as unknown as Promise<{ likes: number; dislikes: number; myReaction: "LIKE" | "DISLIKE" | "" }[]>,
    sort === "top"
