...
import { getUserListIds, getUserProfile } from "@/lib/movies-store";
...
export default async function ProfilePage() {
  ...
  const [user, favorites, later, movies] = await Promise.all([
    getUserProfile(session.user.id), getUserListIds(session.user.id, "FAVORITE"), getUserListIds(session.user.id, "WATCH_LATER"), getAllMovies(),
  ]);
