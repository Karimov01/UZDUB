...
export default function ComingSoonCard({ item }: { item: MovieCardData }) {
  ...
  const content = <>
    <div className="relative aspect-[2/3] overflow-hidden rounded-xl border border-white/15 bg-[#111315]">
      {item.posterUrl ? <Image src={optimizedTmdbImage(item.posterUrl, "poster")!} alt={`${item.title} posteri`} fill className="object-cover transition-transform duration-500 group-hover:scale-[1.025]" sizes="(max-width: 640px) 46vw, 220px" /> : <div className="absolute inset-0 flex items-center justify-center text-white/25"><ImageOff className="h-9 w-9" /></div>}
  ...
  const classes = "group block w-[calc((100vw-44px)/2)] max-w-[184px] shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-400 lg:w-[calc((min(100vw,1400px)-112px)/6)] lg:max-w-none";
